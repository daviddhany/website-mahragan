...
function setupCollapsibleCards(options = {}) {
  const defaultOpenTitles = options.defaultOpenTitles || [];
  const selectors = options.selectors || ['main > section.card', '#adminOnly > section.card'];
  const cards = document.querySelectorAll(selectors.join(','));

  cards.forEach((card) => {
    if (card.dataset.collapsibleReady === 'true') return;
    if (card.id === 'editBox') return;

    const title = card.querySelector(':scope > h2, :scope > h3');
    if (!title) return;

    const titleText = title.textContent.trim();
    const isOpen = defaultOpenTitles.includes(titleText);
    ...
    btn.type = 'button';
    btn.className = 'collapsible-card-toggle';
    btn.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
    btn.innerHTML = `<span>${escapeHtml(titleText)}</span><span class="collapsible-arrow">${isOpen ? '▲' : '▼'}</span>`;
    ...
    while (card.childNodes.length > 1) {
      content.appendChild(card.childNodes[1]);
    }
